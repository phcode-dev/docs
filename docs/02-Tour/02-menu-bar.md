...
Click any menu to open it. Most items show their default keyboard shortcut next to the label, and toggleable items (like **Word Wrap** or **Line Numbers**) display a checkmark when they are on. To change the shortcut for any item, hover over it and click the keyboard icon that appears on the right.
...
The **File** menu handles your project and the files inside it. From here you can create files and folders, start a new project, open files and folders, close them, save them, reopen recently closed files, and bring up the **Recent Files** dialog.

![File menu opened](./images/file-menu.png "The File menu")

It also has shortcuts to the **Open Preferences File**, **Keyboard Shortcuts...**, **Custom Snippets...**, and **Extension Manager...** dialogs, a **Git** submenu (when the Git extension is enabled), the **Live Preview** controls, and the **Quit** option.

> Some items, such as **Open Files...**, **Save As...**, **New Window**, and **Quit**, only appear in the desktop app.

## Edit

The **Edit** menu has all the text-editing commands. It covers **Undo** and **Redo**, the clipboard actions (**Cut**, **Copy**, **Paste**), and selection helpers like **Select All**, **Select Line**, **Split Selection into Lines**, and **Add Cursor to Previous/Next Line** for multi-cursor editing.
...
You will also find next/previous match navigation, plus **Add Next Occurrence** and **Skip and Add Next Occurrence** for incremental multi-cursor selection.
...
- **Quick Open** (`Ctrl/Cmd + Shift + O`) jumps to any file by typing part of its name.
- **Go to Line** jumps to a specific line number.
- **Quick Edit** (`Ctrl/Cmd + E`) opens an inline editor for related code, such as the CSS rules for an HTML element or a function definition from its call site.
- **Quick Docs** (`F1`) shows inline documentation for the symbol at the cursor.
- **Jump to Definition** and **Quick Find Definition** go straight to where a symbol is defined.
- **Go to Next Problem** and **Go to Previous Problem** step through linting errors and warnings.
- **Show in File Tree** reveals the current file in the sidebar.
- **Next Document** and **Previous Document** switch between open files.
